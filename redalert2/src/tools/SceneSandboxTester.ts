...
export class SceneSandboxTester {
    ...
    private static state: SandboxState = {
        ...
        tickMultiplier: 2,
        spawnedCount: 0,
        lastMessage: 'Select a unit, click "Enter placement mode", then left-click on the map to place it.',
    };
    private static readonly fallbackObjectDisplayNames: Record<string, string> = {
        ...
        GAAIRC: 'Air Force Command HQ',
        NARADR: 'Radar Tower',
        GAYARD: 'Allied Naval Shipyard',
        NAYARD: 'Soviet Naval Shipyard',
        YAYARD: 'Yuri Naval Shipyard',
    ...
    static async main(_mixFileLoader: any, gameMapFile: any, parentElement: HTMLElement, strings: StringsLike, context: TestToolRuntimeContext = {}, options: SceneSandboxOptions = {}): Promise<void> {
        ...
        // RA2 style: left-click to move, right-click drag to scroll the map (same as the real game default).
        ...
        this.state = {
            ...
            spawnedCount: 0,
            lastMessage: `Loaded ${options.mapName ?? 'test map'}. Select a unit on the left, then enter placement mode.`,
        };
        ...
        const handleShiftPlacementMouseUp = (event: MouseEvent) => {
            ...
            if (!tile) {
                this.setStatus('Shift held: no placeable map tile found.');
                return;
            }
            const spawnedCount = this.spawnAt(tile);
            if (spawnedCount > 0) {
                this.setStatus('Shift held: unit placed. Release Shift to restore normal interaction.');
            }
        };
        ...
        const handlePlacementClick = (event: any) => {
            ...
            if (this.state.demolitionActive) {
                ...
                if (!building) {
                    this.setStatus('Demolition failed: no demolishable building at this position.');
            ...
            if (this.state.superWeaponTargetingActive) {
                ...
                if (!tile) {
                    this.setStatus('Superweapon targeting failed: no map tile found.');
                    return;
                }
            ...
            if (!this.shiftPlacementActive) {
                this.setPlacementActive(false, spawnedCount > 0
                    ? 'Placed and exited placement mode: you can right-click to move/attack or box-select other units.'
                    : 'Exited placement mode: select units normally and right-click to move/attack.');
            }
            else if (this.shiftPlacementActive && spawnedCount > 0) {
                this.setStatus('Shift held: temporary placement mode continues.');
            }
        };
        ...
        const handleKeyDown = (event: KeyboardEvent) => {
            ...
            if (!this.state.placementActive) {
                ...
                this.setPlacementActive(true, 'Shift held: temporary placement mode enabled.');
            }
        };
        const handleKeyUp = (event: KeyboardEvent) => {
            ...
            if (this.shiftPlacementActive) {
                this.shiftPlacementActive = false;
                this.setPlacementActive(false, 'Shift released: normal interaction restored.');
            }
        };
    ...
    private static buildControlPanel(host: HTMLElement, catalog: Record<SpawnKind, string[]>, mapName: string): HTMLDivElement {
        ...
        [
            ...
            ['aircraft', 'Aircraft'],
            ['building', 'Buildings'],
            ['superweapon', 'Superweapon buildings'],
        ].forEach(([value, label]) => {
        ...
        for (const multiplier of this.tickMultipliers) {
            const button = this.createButton(`${multiplier}x`, () => {
                ...
                this.setStatus(`Speed multiplier set to ${multiplier}x.`);
                this.syncControls();
            });
        ...
        const clearButton = this.createButton('Clear spawned objects', () => this.clearSpawnedObjects());
        ...
        row('Owned superweapons', superWeaponSelect);

        const superWeaponButton = this.createButton('Trigger superweapon (W)', () => {
    ...
    private static getQuickPresets(catalog: Record<SpawnKind, string[]>): SpawnPreset[] {
        ...
        const presets: SpawnPreset[] = [
            ...
            { label: 'Superweapon', kind: 'superweapon', name: pick('superweapon', ['NAMISL', 'NAMSLO', 'NAIRON', 'GACSPH', 'GAWEAT', 'YAPPET']) },
        ];
    ...
    private static syncControls(): void {
        ...
        superWeaponButton.textContent = this.state.superWeaponTargetingActive ? 'Cancel superweapon (W)' : 'Trigger superweapon (W)';
    ...
    private static getSuperWeaponDisplayName(name: string): string {
        const labels: Record<string, string> = {
            NukeSpecial: 'Nuclear Missile',
            IronCurtainSpecial: 'Iron Curtain',
            ChronoSphereSpecial: 'Chronosphere',
    ...
    private static triggerSuperWeaponAt(tile: any): void {
        ...
        if (!runtime || !superWeapon) {
            this.setSuperWeaponTargetingActive(false, 'No superweapon available to trigger.');
            return;
        }
        const rules = superWeapon.rules;
        if (rules?.type === undefined) {
            this.setSuperWeaponTargetingActive(false, `Superweapon ${superWeapon.name} is missing Type and cannot be triggered.`);
            return;
        }
        if (rules.preClick && !this.pendingSuperWeaponTile) {
            this.pendingSuperWeaponTile = tile;
            this.setStatus(`First target selected @ ${tile.rx},${tile.ry}; left-click to select the second target.`);
            return;
        }
    ...
    private static handleSandboxShortcut(event: KeyboardEvent): boolean {
        ...
        if (event.key === 'Escape' && (this.state.placementActive || this.state.demolitionActive || this.state.superWeaponTargetingActive)) {
            ...
            this.setSuperWeaponTargetingActive(false, 'Current temporary mode canceled.');
            return true;
        }
        ...
        if (key === 'p') {
            ...
            this.setStatus('Selected own combat units.');
            return true;
        }
        if (key === 'w') {
            event.preventDefault();
    ...
    private static executeKeyboardOrder(orderType: OrderType): void {
        ...
        if (!selectedUnits.length) {
            this.setStatus(`${OrderType[orderType]} not executed: no own units selected.`);
            return;
        }
        ...
        this.setStatus(`Executed hotkey command: ${OrderType[orderType]}.`);
    }
    ...
    private static centerOnHome(): void {
        ...
        if (!tile) {
            this.setStatus('H (return to base) failed: no own building or unit found.');
            return;
        }
        ...
        this.setStatus(`Returned camera to base @ ${tile.rx},${tile.ry}.`);
    }
    ...
    private static setPlacementActive(active: boolean, message?: string): void {
        ...
        this.setStatus(message ?? (active
            ? 'Placement mode enabled: left-click on the map to place, right-click or use the button to exit.'
    ...
    private static setSuperWeaponTargetingActive(active: boolean, message?: string): void {
        ...
        if (active && !available.length) {
            this.setStatus('No superweapon available: place a superweapon building first.');
            this.syncControls();
            return;
        }
        if (active) {
        ...
        this.setStatus(message ?? (active
            ? 'Superweapon targeting: left-click to select a target, right-click to cancel.'
            : 'Exited superweapon targeting.'));
        this.syncControls();
    }
    ...
    private static setDemolitionActive(active: boolean, message?: string): void {
        ...
        this.setStatus(message ?? (active
            ? 'Demolish mode enabled: left-click a building to trigger the demolition animation; right-click or Esc to exit.'
            : 'Demolish mode disabled.'));
        this.syncControls();
    }
    ...
    private static demolishBuilding(building: any): void {
        ...
        try {
            ...
            worker.unplace(building, () => {
                ...
                this.setStatus(`Finished demolishing ${objectLabel} (${building.name}).`);
                this.syncControls();
            });
    ...
    private static executeLocalOrder(event: { orderType: number; target: any; feedbackType?: unknown }): void {
        ...
        try {
            ...
            if (acceptedUnits.length) {
                runtime.orderAcceptedSerial += 1;
                this.setStatus(`Order issued to ${acceptedUnits.length} units.`);
            }
            else {
                this.setStatus(`Order not accepted: ${selectedUnits.length} own units selected.`);
            }
        }
        catch (error) {
    ...
    private static spawnAt(targetTile: any): number {
        ...
        for (let index = 0; index < this.state.count; index += 1) {
            ...
            if (!tile) {
                unit.dispose?.();
                this.setStatus(`Could not find a placeable position near ${targetTile.rx},${targetTile.ry}. Spawned ${spawned.length}/${this.state.count}.`);
                break;
            }
            if (unit.isInfantry?.()) {
        ...
        this.setStatus(`Spawned ${spawned.length} x ${objectLabel} (${this.state.objectName}) @ ${targetTile.rx},${targetTile.ry}.`);
        this.keepSandboxPlayersActive(runtime.game, runtime.localPlayer, runtime.enemyPlayer);
    ...
    private static spawnBuildingAt(targetTile: any): number {
        ...
        for (let index = 0; index < this.state.count; index += 1) {
            ...
            if (!tile) {
                this.setStatus(`Could not find a placeable building position near ${targetTile.rx},${targetTile.ry}. Spawned ${spawned.length}/${this.state.count}.`);
                break;
            }
        ...
        statusEl.textContent = [
            ...
            this.state.placementActive
                ...
                : this.state.demolitionActive
                    ? 'Mode: demolish'
                    : this.state.superWeaponTargetingActive
                        ? 'Mode: superweapon targeting'
                        : 'Mode: normal interaction',
        ].join('\n');
    }
