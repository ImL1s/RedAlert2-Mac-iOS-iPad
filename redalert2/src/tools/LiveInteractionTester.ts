...
export class LiveInteractionTester {
    ...
    static async main(_mixFileLoader: any, gameMapFile: any, parentElement: HTMLElement, strings: StringsLike, context: TestToolRuntimeContext = {}, deps: LiveInteractionRuntimeDeps = {}): Promise<void> {
        ...
        try {
            ...
            this.appendLog('System', 'Live interaction mode loaded. Use the mock buttons on the right to verify behavior.');
    ...
    private static buildLayout(root: HTMLElement, strings: StringsLike): void {
        ...
        minimapBadge.textContent = 'Real minimap';
        ...
        subtitle.textContent = 'This directly reuses the real skirmish world rendering, interaction, and minimap components, only hiding the sidebar and bottom bar. The red home base is at the top and the blue home base is at the bottom; reinforcements on both sides move toward and attack the enemy home base by default. Use screen-edge scrolling, right-click drag, wheel zoom, and minimap jumps to view the battlefield.';
        ...
        const codeInput = this.buildLabeledInput(credentialsGrid, 'Identity code', 'code');
        ...
        mockTitle.textContent = 'Local testing';
        ...
        mockButtons.append(
            ...
            this.buildButton('Battlefield overview', 'focus-center'),
        );
        ...
        danmakuInput.placeholder = 'Custom danmaku, e.g. blue bottom push / red top defend';
        ...
        mappingSummary.textContent = [
            'Event mapping',
            'Room enter / like -> red reinforcements (top)',
            'Gift / guard / super chat -> blue reinforcements (bottom)',
            'Danmaku containing red/top -> red top lane',
            'Danmaku containing blue/bottom -> blue bottom lane',
            'Heavy and elite units show viewer nicknames first',
        ].join('\n');
    ...
    private static createLoadingScreenPlayers(): Array<{
        ...
        return [
            {
                name: 'Red home base (top)',
                countryId: topCountryId,
                colorId: 0,
                teamId: NO_TEAM_ID,
            },
            {
                name: 'Blue home base (bottom)',
    ...
    private static async initializeBattle(gameMapFile: any, strings: StringsLike, context: TestToolRuntimeContext, deps: LiveInteractionRuntimeDeps): Promise<BattleContext> {
        ...
        // WorldScene.updateCamera now treats pan as zoom-1 screen pixels natively,
        // so the old pan*zoom compensation wrapper is no longer needed here.
        ...
        const leftBase = this.spawnHomeBase(game, leftPlayer, leftBaseName, leftBaseTile, 'Red home base');
        const rightBase = this.spawnHomeBase(game, rightPlayer, rightBaseName, rightBaseTile, 'Blue home base');
        ...
        this.ui.catalogSummary!.textContent = [
            'Unit mapping',
            `Red home base (top): ${leftBaseName}`,
            `Blue home base (bottom): ${rightBaseName}`,
    ...
    private static updatePanelLayout(viewport: { width: number; height: number; }): void {
        ...
        if (this.ui.panelToggle) {
            ...
            this.ui.panelToggle.title = this.panelCollapsed ? 'Expand right toolbar' : 'Collapse toolbar to the right';
        }
    }
    ...
    private static updateBaseLabels(): void {
        ...
        const leftBaseStatus = this.getBaseStatus(this.battle.leftBase, 'Red home base');
        const rightBaseStatus = this.getBaseStatus(this.battle.rightBase, 'Blue home base');
    ...
    private static renderBattleLabels(): void {
        ...
        const leftBaseStatus = this.getBaseStatus(battle.leftBase, 'Red home base');
        const rightBaseStatus = this.getBaseStatus(battle.rightBase, 'Blue home base');
        ...
        try {
            ...
            eventSource.onerror = () => {
                this.state.runtimeStatus = {
                    ...
                    connected: false,
                    sessionActive: false,
                    lastError: 'Event stream connection lost. Make sure the local runtime is still running.',
                };
                this.syncState();
            };
    ...
    private static async handleDisconnect(): Promise<void> {
        try {
            ...
            this.state.runtimeStatus = status as RuntimeStatus;
            this.appendLog('System', 'Live interaction connection closed.');
        } catch (error: any) {
    ...
    private static async handleUiAction(action: string): Promise<void> {
        switch (action) {
            ...
            case 'mock-super-chat':
                await this.emitMockEvent('super-chat', { uname: 'SuperChatUser', price: 30, totalPrice: 3000, message: 'blue charge' });
                return;
            case 'mock-danmaku-left':
                await this.emitMockEvent('danmaku', { uname: 'DanmakuTrooper', message: 'red top top top' });
                return;
            case 'mock-danmaku-right':
                await this.emitMockEvent('danmaku', { uname: 'DanmakuTrooper', message: 'blue charge' });
                return;
    ...
    private static handleInteractionEvent(event: NormalizedInteractionEvent): void {
        ...
        if (!plan) {
            this.appendLog('Event', `${event.cmd} received, but it is not configured to spawn units.`);
            this.syncState();
            return;
        }
        this.spawnWave(plan);
    ...
    private static resolveWavePlan(event: NormalizedInteractionEvent): WavePlan | null {
        ...
        switch (event.kind) {
            ...
            case 'danmaku': {
                ...
                const side = message.includes('blue') || message.includes('bottom') || message.includes('right') || message.includes('blue') || message.includes('bottom')
                    ? 'right'
                    : 'left';
                const premium = message.includes('tank') || message.includes('tank');
                return premium
            ...
            case 'live-start':
                return { side: 'left', reason: 'stream start', infantryBasic: 3, vehicleLight: 1, viewerLabel };
            case 'live-end':
                return { side: 'right', reason: 'stream end', infantryElite: 3, vehicleHeavy: 1, viewerLabel };
            default:
                return null;
        }
    }
    ...
    private static focusCenter(withLog: boolean = true): void {
        ...
        this.syncState();
        if (withLog) {
            this.appendLog('System', 'Camera switched to battlefield overview; drag to keep inspecting details.');
        }
    }
    ...
    private static describeEvent(event: NormalizedInteractionEvent, plan: WavePlan): string {
        ...
        return `${source} triggered ${plan.reason}; ${sideLabel} spawns: ${units.join(' / ')}`;
    }
    ...
    private static syncState(): void {
            ...
            this.ui.statusBadge.textContent = runtimeStatus.lastError
                ...
                : runtimeStatus.sessionActive
                    ...
                    : runtimeStatus.connected
                        ? `Status: ${runtimeStatus.mode === 'mock' ? 'local mock ready' : 'connecting to live'}`
        ...
        if (this.ui.statusSummary) {
            this.ui.statusSummary.textContent = [
                ...
                `Connection: ${runtimeStatus.connected ? 'connected' : 'disconnected'}`,
                `Session: ${runtimeStatus.sessionActive ? 'active' : 'not started'}`,
                `Room: ${runtimeStatus.anchor?.roomId ?? '-'}`,
                `Streamer: ${runtimeStatus.anchor?.uname ?? '-'}`,
        ...
        if (this.ui.leftSummary) {
            this.ui.leftSummary.textContent = [
                ...
                `Units alive: ${leftAlive}`,
                `Total spawned: ${this.state.left.totalSpawned}`,
                `Total lost: ${leftLost}`,
        ...
        if (this.ui.rightSummary) {
            this.ui.rightSummary.textContent = [
                'Blue home base (bottom)',
                `Base HP: ${rightBaseStatus.hpDisplay}`,
                `Units alive: ${rightAlive}`,
                `Total spawned: ${this.state.right.totalSpawned}`,
    ...
    private static getDebugSnapshot(): Record<string, unknown> {
        ...
        return {
            ...
            left: {
                ...
                baseHealth: this.getBaseStatus(battle?.leftBase, 'Red home base'),
            },
            right: {
                totalSpawned: this.state.right.totalSpawned,
                aliveUnits: battle ? this.countAliveUnits(battle.rightPlayer) : 0,
                unitsLost: battle ? battle.rightPlayer.getUnitsLost() : 0,
                baseHealth: this.getBaseStatus(battle?.rightBase, 'Blue home base'),
            },
            lastEvent: this.state.lastEvent,
            recentEvents: this.state.recentEvents,
        };
