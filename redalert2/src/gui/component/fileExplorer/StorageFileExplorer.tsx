...
export const StorageFileExplorer: React.FC<StorageFileExplorerProps> = ({
        ...
        try {
            ...
            for (const file of files) {
                ...
                try {
                    ...
                    shouldOverwrite = await requestConfirm(`The file "${fileName}" already exists. Overwrite it?`, 'Overwrite', 'Cancel');
                }
                catch {
                }
                if (!shouldOverwrite) {
            ...
            if (skippedFiles.length) {
                await showMessage(`The following files are not allowed in the current folder:\n${skippedFiles.join('\n')}`, 'Upload Skipped');
            }
            if (modified) {
                emitInfo(`Uploaded ${files.length - skippedFiles.length} file(s)`);
                onFileSystemChange?.();
                await refresh();
            }
        }
    ...
    const handleRename = async (entry?: ExplorerEntry) => {
        ...
        if (!targetEntry.canModify) {
            await showMessage('This item cannot be renamed.');
            return;
        }
        const originalPath = buildPath(currentSegments, targetEntry.id);
        const nextNameInput = (await requestText(`Enter a new name for "${targetEntry.name}":`))?.trim();
    ...
    const handleCreateFolder = async () => {
        if (canCreateFolder && !canCreateFolder(currentPath, currentSegments)) {
            await showMessage('New folders cannot be created in the current folder.');
            return;
        }
        const folderName = (await requestText('Enter a name for the new folder:'))?.trim();
        if (!folderName) {
            return;
        }
    ...
    const handleDelete = async (entriesToDelete: ExplorerEntry[] = selectedEntries) => {
        ...
        if (systemFiles.length) {
            const confirmedSystemDelete = await requestConfirm(
                `"${systemFiles.map((path) => path.split('/').pop()).join(', ')}" are system files. Deleting them may prevent the game from working correctly.\n\nAre you sure you want to continue?`,
                'Delete',
                'Cancel',
            );
        ...
        const confirmedDelete = await requestConfirm(
            `Are you sure you want to permanently delete these ${entriesToDelete.length} item(s)?`,
            'Delete',
            'Cancel',
        );
        if (!confirmedDelete) {
        ...
        try {
            ...
            emitInfo(`Deleted ${entriesToDelete.length} item(s)`);
            onFileSystemChange?.();
            await refresh();
        }
    ...
    const handleDownload = async (entriesToDownload: ExplorerEntry[] = selectedEntries) => {
        ...
        try {
            ...
            if (entriesToDownload.length > 1 || entriesToDownload[0]?.type === 'folder') {
                ...
                emitInfo(`Downloaded ${entriesToDownload.length} item(s) as an archive`);
                return;
            }
    ...
    const handleClipboardStage = (mode: 'copy' | 'cut', entriesToStage: ExplorerEntry[] = selectedEntries) => {
        ...
        emitInfo(`${mode === 'copy' ? 'Copied' : 'Cut'} ${entriesToStage.length} item(s)`);
    };
    ...
    const transferEntries = async (payload: InternalDragPayload, mode: 'copy' | 'cut', targetSegments: string[]) => {
        ...
        if (mode === 'cut' && sourcePath === targetPath) {
            emitInfo('Source and destination folders are the same; nothing was moved');
            return;
        }
        try {
            setLoading(true);
            ...
            for (const item of payload.items) {
                ...
                if (mode === 'copy' && sourcePath === targetPath) {
                    ...
                    const overwrite = await requestConfirm(`"${targetName}" already exists. Overwrite it?`, 'Overwrite', 'Cancel');
                    if (!overwrite) {
                        continue;
                    }
            ...
            if (modified) {
                emitInfo(`${mode === 'copy' ? 'Pasted' : 'Moved'} ${payload.items.length} item(s)`);
                onFileSystemChange?.();
    ...
    const handleClipboardEvent = (event: React.ClipboardEvent<HTMLDivElement>, mode: 'copy' | 'cut') => {
        ...
        emitInfo(`${mode === 'copy' ? 'Copied' : 'Cut'} ${selectedEntries.length} item(s)`);
    };
    ...
    const statusSegments = [
        selectedEntries.length ? `${selectedEntries.length} item(s) selected` : `${entries.length} item(s)`,
        currentPath,
        clipboard ? `${clipboard.mode === 'copy' ? 'Copied' : 'Cut'}: ${clipboard.items.length} item(s)` : '',
        error || statusMessage || '',
    ].filter(Boolean);
                    ...
                    <div className="fe_fileexplorer_body_wrap">
                        ...
                        >
                            <div className="fe_fileexplorer_items_scroll_wrap_inner">
                                            ...
                                            <div className="fe_fileexplorer_items_clipboard_overlay_paste_text_wrap">
                                                <div className="fe_fileexplorer_items_clipboard_overlay_paste_text">
                                                    <div className="fe_fileexplorer_items_clipboard_overlay_paste_text_big">
                                                        Press {pasteShortcutLabel} to paste into the current folder
                                                    </div>
                                ...
                                {dragActive ? (
                                    <div className="fe_fileexplorer_drop_message">Drop files to upload to the current folder</div>
                                ) : null}
                                {loading ? (
