...
export function formatSdpCandidateSummary(summary: SdpCandidateSummary): string {
    const parts = [
        `${summary.totalCandidates} candidate(s)`,
...
export function getSdpCandidateWarning(summary: SdpCandidateSummary): string | undefined {
    if (!summary.totalCandidates) {
        return 'This SDP contains no ICE candidates; a direct LAN connection between machines cannot be established.';
    }
    if (summary.hasMdnsHostCandidate &&
        !summary.hasPrivateIpv4Candidate &&
        !summary.hasSrflxCandidate &&
        !summary.hasRelayCandidate) {
        return 'The browser only exposed an mDNS host candidate (*.local), with no LAN IPv4/srflx/relay candidates; same-machine or 127.0.0.1 connections usually work, but cross-machine LAN connections often fail because mDNS/UDP is blocked.';
    }
    return undefined;
}
