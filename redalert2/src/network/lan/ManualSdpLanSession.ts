...
export class ManualSdpLanSession {
    ...
    async acceptGuestAnswer(answerText: string): Promise<void> {
        if (!this.peerConnection || this.snapshot.role !== 'host') {
            throw new Error('Generate an offer in host mode first.');
        }
        ...
        this.snapshot.remoteDescriptionApplied = true;
        this.refreshSnapshot();
        this.log('info', 'Answer imported. Waiting for the data channel to open.');
    }
    ...
    sendChat(text: string): void {
        ...
        if (!this.dataChannel || this.dataChannel.readyState !== 'open') {
            throw new Error('The data channel is not open yet; messages cannot be sent.');
        }
    ...
    private bindPeerEvents(pc: RTCPeerConnection): void {
        pc.addEventListener('connectionstatechange', () => {
            ...
            this.refreshSnapshot();
            this.log('info', `Connection state changed to ${pc.connectionState}.`);
        });
        pc.addEventListener('iceconnectionstatechange', () => {
            ...
            this.refreshSnapshot();
            this.log('info', `ICE connection state changed to ${pc.iceConnectionState}.`);
        });
    ...
    private attachDataChannel(channel: RTCDataChannel, source: 'local' | 'remote'): void {
        ...
        channel.addEventListener('open', () => {
            ...
            this.refreshSnapshot();
            this.log('info', 'Data channel open. You can now send test messages.');
            try {
                this.sendEnvelope({
                    type: 'hello',
    ...
    private handleEnvelopeText(text: string): void {
        ...
        if (payload.type === 'hello') {
            this.log('info', `${this.getRemoteLabel()} completed the handshake.`);
            return;
        }
    ...
    private sendEnvelope(payload: ManualLanEnvelope): void {
        if (!this.dataChannel || this.dataChannel.readyState !== 'open') {
            throw new Error('The data channel is not open yet.');
        }
    ...
    private parseDescription(text: string, expectedType: RTCSdpType): RTCSessionDescriptionInit {
        ...
        if (!parsed || typeof parsed !== 'object') {
            throw new Error('Description text is malformed.');
        }

        const candidate = parsed as RTCSessionDescriptionInit;
        if (candidate.type !== expectedType) {
            throw new Error(`Expected a ${expectedType}, but the provided text is of type ${candidate.type ?? 'unknown'}.`);
        }
